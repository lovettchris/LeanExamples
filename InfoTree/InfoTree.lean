...
partial def getInfoTree (fileName: String) : IO Unit := do
  let contents ← IO.FS.readFile fileName

  let fileMap : FileMap := FileMap.ofString contents
  let caps : ClientCapabilities := {}

  let context := Parser.mkInputContext contents fileName
  ...
  -- IO.println s!"Header: {environment.header.mainModule}"
  -- IO.println s!"Header: {environment.header.moduleNames}"
  if messages.hasErrors then
    for msg in messages.toList do
      IO.println (← msg.toString)

  -- environment.displayStats
